...
from user_interface.align import Align  # オブジェクトの位置を設定するクラ
from user_interface import colors  # 色の定数と補色を求める関数
...
def main():
    ...
    # 色設定(r, g, b)
    while(True):
        screen.fill(colors.AQUA)  # ウィンドウを塗りつぶす
        # 描画する文字を設定(文字列, アンチエイリアスの有無, 色)
        text_title = font.render('Game Title', True, colors.comp_color(colors.AQUA))
        # 文字の表示位置を設定
        aling = Align(text_title, WIDTH, HEIGHT)  # タイトルの表示位置を操作する準備
