...
class State:
    ...
    def transition(self):
        ...
        for event in pygame.event.get():
            ...
            elif self.state == States.TITLE and event.type == KEYDOWN:
                if event.key == K_SPACE:
                    self.state = States.CHOOSE  # 次の状態CHOOSEに遷移する
            # 状態がCHOOSEでキーの押下があったとき
            elif self.state == States.CHOOSE and event.type == KEYDOWN:
                if event.key == K_SPACE:
                    self.state = States.PLAY  # 次の状態PLAYに遷移する
            # 状態がPLAYでキーの押下があったとき
            elif self.state == States.PLAY and event.type == KEYDOWN:
                if event.key == K_SPACE:
...
class Drawer(State):
    ...
    def choose(self) -> None:
        ...
        text_p2 = self.font_M.render(
            'むずかしい: Input " j "', True, Color.WHITE.rgb
        )  # 難易度2
